...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(prog='meas_0_plot', description='Process MSR2MRP border node related stats',
                                     epilog=':-(')
    ...
    axs = [ax1, ax2]
    for nw_filename, ax, ratio, anchor, sensor, path, t in zip([args.network1] + [args.network2], axs, [1, 1],
                                                               ['S', 'N'], [args.sensor1, args.sensor2],
                                                               [args.path1, args.path2], title):
        ...
        colors = ['tab:blue' if 'external' in i[0][1] else 'tab:brown' if 'border' in i[0][1] else 'tab:pink' for i in
                  nx.get_node_attributes(nw, 'roles').values()]
        ...
        if sensor is not None:
            ...
            if path is not None:
                ...
                paths = [j for i in nw.out_edges(sensor, data=True) for j in i[2]['pathid']]
            ...
            for p in paths:
                ...
                for i in nw.edges(data=True):
                    if not (p in i[2]['pathid'] or 0 in i[2]['pathid']):
                ...
                e_set = e_set.union([e for e in edges if e in pathids])
                n_set = n_set.union([i for e in edges if e in pathids for i in e])

            n_alpha = [1 if i in n_set else 0.2 for i in nw]
            e_alpha = [1 if i in e_set else 0.2 for i in nx.get_edge_attributes(nw, 'pathid')]
        else:
            n_alpha = 1
            e_alpha = 1
        ...
        ax.axis('off')
        ax.set_title(t, loc='left', pad=5, x=0.05)
